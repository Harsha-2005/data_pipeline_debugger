...
<head>
    ...
    <style>
        ...
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .modal-title { font-size: 1.25rem; font-weight: 700; margin-bottom: 1rem; border-bottom: 1px solid var(--border-color); padding-bottom: 0.5rem; }
        .modal-list { list-style: none; padding: 0; margin: 0; max-height: 300px; overflow-y: auto; }
        .modal-list li { padding: 0.5rem 0; border-bottom: 1px solid var(--border-color); font-size: 0.9rem; }
...
<body>
    ...
    <div class="page-wrapper">
        ...
        <!-- Stats Row -->
        <div class="stats-row">
            <div class="stat-card" onclick="showStatModal('agents', 'Agents')"><div class="stat-num">6</div><div class="stat-label">Agents</div></div>
            <div class="stat-card" onclick="showStatModal('levels', 'Difficulty Levels')"><div class="stat-num">5</div><div class="stat-label">Difficulty Levels</div></div>
            <div class="stat-card" onclick="showStatModal('actions', 'Action Space')"><div class="stat-num">11</div><div class="stat-label">Action Space</div></div>
            <div class="stat-card" onclick="showStatModal('multi', 'Multi-Agent Roles')"><div class="stat-num">3</div><div class="stat-label">Multi-Agent Roles</div></div>
            <div class="stat-card" onclick="showStatModal('algos', 'Training Algorithms')"><div class="stat-num">5</div><div class="stat-label">Training Algorithms</div></div>
        </div>
    ...
    <div id="stat-modal" class="modal-overlay" onclick="if(event.target===this) closeStatModal()">
        <div class="modal-content">
            ...
            <div class="modal-title" id="stat-modal-title">Details</div>
            <ul class="modal-list" id="stat-modal-list"></ul>
        </div>
    </div>

<script>
const STAT_DATA = {
    'agents': ['Rule-Based Agent', 'LLM Agent (Qwen2.5-72B)', 'Inspector Agent', 'Fixer Agent', 'Validator Agent', 'Curriculum Agent'],
    'levels': ['Easy (task_easy_schema_fix)', 'Medium (task_medium_data_quality)', 'Hard (task_hard_pipeline_orchestration)', 'Very Hard (task_veryhard_streaming_pipeline)', 'Expert (task_expert_multi_source_join)'],
    'actions': ['inspect', 'cast_column', 'drop_nulls', 'fill_nulls', 'drop_duplicates', 'filter_outliers', 'rename_column', 'apply_business_rule', 'reorder_stages', 'validate', 'submit'],
    'multi': ['Inspector Agent', 'Fixer Agent', 'Validator Agent'],
    'algos': ['Curriculum Learning', 'ε-Greedy Exploration', 'Rule-Based Fallback', 'GRPO/PPO Fine-Tuning', 'Adaptive Difficulty']
};

function showStatModal(type, title) {
    document.getElementById('stat-modal-title').textContent = title;
    const ul = document.getElementById('stat-modal-list');
    ul.innerHTML = '';
    (STAT_DATA[type] || []).forEach(item => {
...
function drawChart() {
    ...
    for (let i = 0; i < episodes; i++) {
        ...
        if (score >= thresholds[taskIdx] && taskIdx < 4) { taskIdx++; score -= penalties[taskIdx] * 0.8; }
