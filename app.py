...
@app.get("/api/compete-run")
def compete_run(task_id: str = "task_easy_schema_fix", seed: int = 42):
    """Run two agents independently on the same task and return per-step results."""
    try:
        from env.environment import DataPipelineEnv
        ...
        def run_agent(agent, env_instance):
            ...
            for step_idx in range(max_steps):
                ...
                # Read reward from env.history (Observation has no reward field)
        ...
        env1 = DataPipelineEnv(task_id=task_id, seed=seed)
        env2 = DataPipelineEnv(task_id=task_id, seed=seed)
